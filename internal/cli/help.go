...
var commandDocs = []commandDoc{
	...
	},
	{
		...
		long:    "Runs Fidelity's five-stage pipeline (capture, declare, observe, reconcile, manifest) using a checked policy file. It reads the checkpoint transcript through the parent Entire CLI, grounds direct mentions in the local graph symbol dictionary, resolves checkpoint trailers to commits, and reconciles the resulting semantic diff into six tiers (confirmed, declared_unimplemented, expected_blast_radius, undeclared_scope_creep, advisory_low_confidence, unverifiable_coverage), the last of which never rests on an absent graph path alone when coverage is partial or unknown. Writes verdict.json and VERIFY_REPORT.md to --out, prints a terminal report to stderr, and the full JSON response to stdout.",
		...
		flags: []flagDoc{
			...
			{name: "--out", arg: "path", def: "fidelity-out", desc: "Output directory for verdict.json and VERIFY_REPORT.md"},
			{name: "--repo", arg: "path", desc: "Repository (default: current repo)"},
		},
